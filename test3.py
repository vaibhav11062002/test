...
GEMINI_API_KEY = "AIzaSyDDrwMUTp75A3Dc64auV-SHSv402mS1w4M"
...
Price recommendations = average realized price adjusted for margin sustainability and competitive gap.

Retention offers → clearly state value proposition versus competitors (price, reliability, supply, service).
...
def cluster_one_company(g: pd.DataFrame, raw_df: pd.DataFrame, customer_col: str, item_col: str) -> pd.DataFrame:
    # Create pivot table: customers x materials with sum of revenue, fill NaNs with 0
    pivot = raw_df.groupby([customer_col, item_col])[REVENUE_COL].sum(min_count=1).unstack(fill_value=0).fillna(0)
    
    customers = g[customer_col].unique()
    pivot = pivot.loc[pivot.index.isin(customers)]

    # Handle case where there are < 3 customers (can't run KMeans with n_clusters=3)
    if len(pivot) < 3:
        g = g.copy()
        ranks = g["total_revenue"].rank(method="first", ascending=False)
        labels = np.where(
    ...
    # Log transform and replace any NaN/inf values with 0
    X = np.log1p(pivot.values)
    X = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)

    km = KMeans(n_clusters=3, n_init=50, random_state=42)
    cluster_ids = km.fit_predict(X)

    cluster_map = {cust: cid for cust, cid in zip(pivot.index, cluster_ids)}

    g = g.copy()
    g["cluster_id"] = g[customer_col].map(cluster_map).fillna(-1).astype(int)

    # Order clusters by mean total revenue descending
    means = g.groupby("cluster_id")["total_revenue"].mean().sort_values(ascending=False)
    order = {cid: idx for idx, cid in enumerate(means.index)}
    g["cluster_id"] = g["cluster_id"].map(order).fillna(2).astype(int)
...
try:
    ...
    # Determine item column for material-based clustering
    item_col = None
    if "Item Description" in raw_df.columns:
        item_col = "Item Description"
    elif "Material Group" in raw_df.columns:
        item_col = "Material Group"
    else:
        raise ValueError("No suitable item/material column found for clustering")

    clustered_list = []
    for _, g in agg.groupby(company_col, dropna=False):
        clustered_list.append(cluster_one_company(g, raw_df, CUSTOMER_COL, item_col))
